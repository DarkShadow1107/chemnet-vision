...
| **Stack Tehnologic**                     | Python / TypeScript (Mixt)                       |
| **Domeniul Industrial de Interes (DII)** | Medical / Pharmaceutical (Chimie Computationala) |
| **Tip Retea Neuronala**                  | CNN + MLP + GNN + LSTM (Multimodal)              |
...
| **UI - feedback vizual** | Text simplu                      | Indicator sursa (AI/RAG/DB) + afisare confidence | Transparenta pentru utilizator privind provenienta informatiilor       |
...
Screenshot-ul demonstreaza interfata chat ChemNet-Vision unde utilizatorul interogheaza "Describe Caffeine" si primeste raspunsul corect: "CAFFEINE is a Small molecule. Formula: C8H10N4O2. Weight: 194.19 g/mol." insotit de vizualizarea 2D a structurii moleculare si indicator de sursa "(Information verified by ChemNet Database)".

### 7.3 Demonstratie Functionala End-to-End

**Locatie dovada:** `docs/demo/demo_workflow.md`

**Fluxul demonstrat:**

| Pas | Actiune                                                                                   | Rezultat Vizibil                                                          |
| --- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| 1   | Utilizatorul scrie "Describe Caffeine" in interfata chat                                  | Mesajul apare in zona de input a UI-ului Next.js                          |
| 2   | Sistemul cauta CAFFEINE in baza de date indexata (42k molecule)                           | Gasit -- prepara datele factuale din ChEMBL                               |
| 3   | Modelul AI LSTM genereaza textul descriptiv                                               | Procesare in backend Flask pe portul 5000                                 |
| 4   | Datele factuale din DB suprascriu predictiile AI acolo unde exista diferente              | "CAFFEINE is a Small molecule. Formula: C8H10N4O2. Weight: 194.19 g/mol." |
| 5   | Vizualizarile 2D (PNG) si 3D (SDF) sunt generate din SMILES: `Cn1c(=O)c2c(ncn2C)n(C)c1=O` | Imagine 2D si model 3D interactiv afisate in browser                      |
| 6   | Raspunsul complet este returnat cu metadata sursa                                         | "(Information verified by ChemNet Database)"                              |

**Latenta masurata end-to-end:** ~35ms
**Tehnologii utilizate:** Next.js 16 (frontend, port 3000) + Flask (backend, port 5000) + PyTorch (inferenta) + RDKit (vizualizare) + FAISS (RAG)
...
chemnet-vision/
|
|-- NUME_Prenume_Grupa_README_Proiect_RN.md   # ACEST FISIER (README Final Proiect RN)
